...
import { CaseStudyDetail } from "@/components/content/case-study-detail";
...
export async function generateStaticParams() {
  const [sanitySlugs, staticSlugs] = await Promise.all([getProjectSlugs(), getCaseStudySlugs()]);
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!study) return {};
  const description = getCaseStudySummary(study);
  const cover = getCaseStudyImages(study)[0];
  return {
    title: study.title,
    description,
    ...
    openGraph: {
      ...
      images: cover ? [{ url: absoluteUrl(cover.src) }] : undefined,
    },
  };
}
  ...
  return (
    <>
      <JsonLd
        data={creativeWorkJsonLd({
          title: study.title,
          description: getCaseStudySummary(study),
          slug: study.slug,
      ...
      <PageIntro
        title={study.title}
        description={`${study.label} · ${study.sector} · ${study.location}`}
      />
